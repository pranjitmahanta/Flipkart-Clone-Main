...
const PrimaryDropDownMenu = ({ setTogglePrimaryDropDown, user }) => {
    ...
    const navs = [
        { title: "Supercoin Zone", icon: <OfflineBoltIcon sx={{ fontSize: "18px" }} />, redirect: "/" },
        { title: "Flipkart Plus Zone", icon: <AddCircleIcon sx={{ fontSize: "18px" }} />, redirect: "/" },
        { title: "Orders", icon: <ShoppingBagIcon sx={{ fontSize: "18px" }} />, redirect: "/orders" },
        { title: "Wishlist", icon: <FavoriteIcon sx={{ fontSize: "18px" }} />, redirect: "/wishlist" },
        { title: "My Chats", icon: <ChatIcon sx={{ fontSize: "18px" }} />, redirect: "/" },
        { title: "Coupons", icon: <ConfirmationNumberIcon sx={{ fontSize: "18px" }} />, redirect: "/" },
        { title: "Gift Cards", icon: <AccountBalanceWalletIcon sx={{ fontSize: "18px" }} />, redirect: "/" },
        { title: "Notifications", icon: <NotificationsIcon sx={{ fontSize: "18px" }} />, redirect: "/" },
    ];

    return (
        <div 
            className="absolute w-60 -left-24 ml-2 top-9 bg-white shadow-2xl rounded flex-col text-sm"
            onMouseLeave={() => setTogglePrimaryDropDown(false)} // Close when cursor leaves
        >
            {user.role === "admin" && (
            ...
            {navs.map((item, i) => (
                <Link key={i} className="pl-3 py-3.5 border-b flex gap-3 items-center hover:bg-gray-50" to={item.redirect}>
